...
export default function LearnPage() {
  const categories = [
    ...
    },
    {
      ...
      icon: RotateCcw,
      color: "var(--clr-forced-continuity)",
    ...
    <div className="max-w-5xl mx-auto py-8 animate-fade-in">
      ...
      <div className="text-center mb-16">
        ...
        <h2 className="text-4xl font-extrabold text-white mb-4 tracking-tight">
          What are <span className="gradient-text">Dark Patterns?</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20">
        {categories.map((cat, idx) => {
          const Icon = cat.icon;
          return (
            <GlowCard key={idx} padding="p-0" delay={cat.delay} className={cat.image ? "md:col-span-2 flex flex-col md:flex-row" : "flex flex-col"}>
              
              {/* Image Section (if available) */}
              {cat.image && (
                <div className="w-full md:w-1/2 min-h-[200px] md:min-h-full bg-black/40 relative overflow-hidden border-b md:border-b-0 md:border-r border-white/5">
                  <div className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-r from-transparent via-transparent to-[var(--bg-card)] z-10 opacity-60"></div>
              ...
              <div className={`p-6 flex-1 flex flex-col justify-center`}>
                ...
                <div className="mt-auto pt-4 border-t border-white/5 bg-white/[0.01] -mx-6 -mb-6 p-6">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">Real World Example</p>
      ...
      <div className="relative rounded-3xl p-[1px] overflow-hidden mb-12">
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/30 via-magenta-500/10 to-green-500/30 animate-pulse"></div>
        <div className="relative bg-[var(--bg-card)] backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-white/5">
          <div className="text-center mb-10">
            <ShieldCheck size={40} className="text-green-400 mx-auto mb-4" />
            <h3 className="text-2xl font-bold text-white mb-2">How to Stay Safe</h3>
            <p className="text-sm text-slate-400">Practical tips to protect yourself from manipulative designs.</p>
          </div>
